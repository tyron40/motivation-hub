...
export const [IAPProvider, useIAP] = createContextHook(() => {
  ...
  useEffect(() => {
    const configureRevenueCat = async () => {
      try {
        if (!isNative) {
          console.log('Γä╣∩╕Å RevenueCat disabled on web');
        ...
        if (appOwnership === 'expo') {
          console.log('ΓÜá∩╕Å RevenueCat not available in Expo Go');
          setIsConfigured(false);
          return;
        }

        if (!REVENUECAT_API_KEY) {
          console.error('Γ¥î Missing RevenueCat API key');
          setIsConfigured(false);
          return;
        }
        ...
        console.log('Γ£à RevenueCat configured');
        setIsConfigured(true);
      } catch (error) {
        console.error('Γ¥î Failed to configure RevenueCat:', error);
        setIsConfigured(false);
      }
    };
  ...
  const loadEntitlements = useCallback(async () => {
    try {
      console.log('≡ƒôª Loading entitlements... isAuthenticated:', isAuthenticated, 'isDemoAccount:', isDemoAccount);
      
      if (isDemoAccount) {
        console.log('≡ƒÄ¡ Demo account: Granting unlimited premium access');
      ...
      const timeoutPromise = new Promise<null>((resolve) => {
        setTimeout(() => {
          console.warn('ΓÜá∩╕Å Entitlements loading timeout');
          resolve(null);
        }, 1000);
      });
      ...
      if (stored && typeof stored === 'string') {
        try {
          ...
          console.log('Γ£à Loaded entitlements from storage:', parsed);
          setEntitlements(parsed);
        } catch (parseError) {
          console.error('Γ¥î Error parsing entitlements:', parseError);
          setEntitlements(DEFAULT_ENTITLEMENTS_AUTHENTICATED);
        }
      } else if (isAuthenticated) {
        console.log('Γ£à New authenticated user: Setting default credits to 10');
        setEntitlements(DEFAULT_ENTITLEMENTS_AUTHENTICATED);
      } else {
        console.log('≡ƒæñ No stored entitlements, using defaults');
        setEntitlements(DEFAULT_ENTITLEMENTS_AUTHENTICATED);
      }
    } catch (error) {
      console.error('Γ¥î Error loading entitlements:', error);
      setEntitlements(DEFAULT_ENTITLEMENTS_AUTHENTICATED);
    }
  ...
  const saveEntitlements = useCallback(async (newEntitlements: Entitlements) => {
    try {
      ...
      console.log('Γ£à Entitlements saved:', newEntitlements);
    } catch (error) {
      console.error('Γ¥î Error saving entitlements:', error);
    }
  }, [storageKey]);
  ...
  const purchase = useCallback(async (productId: IAPProductId) => {
    ...
    try {
      ...
      console.error('Γ¥î Purchase failed:', error);
  ...
  const restorePurchases = useCallback(async () => {
    ...
    try {
      ...
      console.error('Γ¥î Restore failed:', error);

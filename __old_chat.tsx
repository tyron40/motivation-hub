...
function ChatScreenContent() {
  ...
  const generateVoice = useCallback(async (messageId: string, text: string) => {
    if (isVoiceMuted) return;
    try {
      console.log('≡ƒÄñ Generating voice for message:', messageId);
      console.log('≡ƒÄñ Using Vercel backend /api/tts endpoint');
      ...
      console.log('Γ£à TTS response received');
      
      const audioUrl = `data:${result.audio.mimeType};base64,${result.audio.base64Data}`;
      console.log('Γ£à Audio URL created');
      
      setMessages(prev => prev.map(msg => 
        msg.id === messageId ? { ...msg, audioUrl } : msg
      ));
      
      console.log('Γ£à Voice generated successfully for message:', messageId);
      
      setTimeout(() => {
        void playAudio(messageId, audioUrl);
      }, 500);
    } catch (error: any) {
      console.error('Γ¥î Voice generation error:', error);
      const errorMsg = error instanceof Error ? error.message : String(error);
      console.error('Γ¥î Error details:', errorMsg);
    }
  ...
  const sendMessage = useCallback(async (text: string, isSuggestion: boolean = false) => {
    ...
    if (!isSuggestion && usageStats.credits <= 0) {
      console.log('Γ¥î No credits available');
      if (Platform.OS !== 'web') {
        Alert.alert(
          'No Credits',
    ...
    if (isFirstRealUserTurn) {
      console.log('≡ƒº¬ First turn outbound payload:', allMessages.map(m => `${m.role}:${m.content.slice(0, 40)}`));
    }
    ...
    try {
      ...
      if (!isSuggestion) {
        ...
        if (!creditUsed) {
          console.log('Γ¥î Failed to deduct credit');
          if (Platform.OS !== 'web') {
            Alert.alert(
              'No Credits',
              'You need credits to use the AI chat feature. Purchase credits to continue.',
        ...
        console.log('Γ£à Credit deducted. Remaining credits:', usageStats.credits - 1);
      } else {
        console.log('Γ£à Suggested question - no credit needed');
      }
      ...
      if (!sessionId) {
        try {
          ...
          console.error('ΓÜá∩╕Å Session create failed (non-blocking):', sessionCreateErr);
        }
      } else {
        try {
          ...
          console.error('ΓÜá∩╕Å Session add (user) failed (non-blocking):', sessionAddErr);
        }
      }

      console.log('≡ƒñû Sending chat message...');
      console.log(
        '≡ƒôñ Messages count:',
        allMessages.length,
        '| isSuggestion:',
      ...
      if (isFirstRealUserTurn && !completion) {
        console.warn('ΓÜá∩╕Å First-turn empty response. Retrying once...');
      ...
      console.log(
        'Γ£à Vercel backend responded, length:',
        completion?.length,
        'keys:',
      ...
      if (isFirstRealUserTurn) {
        console.log('≡ƒº¬ First-turn parse diagnostics:', {
      ...
      if (currentRequestId !== activeRequestIdRef.current) {
        console.log('ΓÜá∩╕Å Ignoring stale chat response for requestId:', currentRequestId);
        return;
      }

      console.log(
        '≡ƒôÑ Assistant parse result | completion length:',
        completion.length,
        '| usedFallback:',
        completion.length === 0
      );
      ...
      if (sessionId) {
        try {
          ...
          console.error('ΓÜá∩╕Å Session add (assistant) failed (non-blocking):', sessionAddErr);
        }
      }
      ...
      if (profile.voiceEnabled && !isVoiceMuted && completion) {
        if (usageStats.credits > 0) {
          ...
          if (voiceCreditUsed) {
            console.log('Γ£à Voice credit deducted. Remaining credits:', usageStats.credits - 1);
            void generateVoice(aiMessage.id, completion);
          } else {
            console.log('ΓÜá∩╕Å Not enough credits for voice generation');
          }
        } else {
          console.log('ΓÜá∩╕Å Not enough credits for voice generation');
        }
      ...
      if (currentRequestId !== activeRequestIdRef.current) {
        console.log('ΓÜá∩╕Å Ignoring stale chat error for requestId:', currentRequestId);
        return;
      }
  ...
  const startRecording = async () => {
    try {
      console.log('≡ƒÄñ Requesting microphone permissions...');
      ...
      console.log('Γ£à Microphone permission granted');
      ...
      console.log('≡ƒÄñ Starting recording...');
      ...
      setIsRecording(true);
      console.log('Γ£à Recording started');
      ...
      console.error('Γ¥î Failed to start recording:', error);
      if (Platform.OS !== 'web') {
  ...
  const stopRecording = async () => {
    if (!recording) {
      console.warn('ΓÜá∩╕Å No recording to stop');
      return;
    }

    try {
      console.log('≡ƒ¢æ Stopping recording...');
      ...
      console.log('Γ£à Recording stopped, URI:', uri);
      ...
      if (uri) {
        ...
        console.error('Γ¥î No recording URI available');
        if (Platform.OS !== 'web') {
          Alert.alert('Error', 'Failed to save recording');
        }
      }
    } catch (error) {
      console.error('Γ¥î Failed to stop recording:', error);
      setIsRecording(false);
      setRecording(null);
      if (Platform.OS !== 'web') {
  ...
  const transcribeAudio = async (uri: string) => {
    try {
      console.log('≡ƒÄ» Starting transcription for URI:', uri);
      ...
      console.log('≡ƒôñ Sending audio to Vercel transcription service...');
      const data = await transcribeAudioApi({ audio: formData });
      console.log('Γ£à Transcription response:', data);

      if (data.text && data.text.trim()) {
        setInputText(data.text.trim());
        console.log('Γ£à Transcription successful:', data.text);
      } else {
        console.warn('ΓÜá∩╕Å Empty transcription result');
        if (Platform.OS !== 'web') {
          Alert.alert('No Speech Detected', 'Please try speaking again.');
        }
      }
    } catch (error) {
      console.error('Γ¥î Transcription error:', error);
      if (Platform.OS !== 'web') {
            ...
            <View style={styles.headerLeft}>
              ...
              <View>
                ...
                <View style={styles.statusIndicator}>
                  ...
                  <Text style={styles.statusText}>Online ΓÇó v{appVersion}</Text>
                </View>
              </View>
            </View>
                ...
                >
                  <View style={styles.historyItemContent}>
                    ...
                    <Text style={styles.historyItemDate}>
                      {formatDate(session.updatedAt)} ΓÇó {session.messages.length} messages
                    </Text>
                  </View>
                  <TouchableOpacity

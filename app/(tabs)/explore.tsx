      ...
      >
        ...
        <View style={[styles.header, { paddingTop: insets.top + 16 }]}>
          ...
          <View style={styles.searchContainer}>
            ...
            <TextInput testID="explore-search-input"
            ...
            {searchQuery.length > 0 && (
              ...
              >
                {isSearchingOnline || isLoading ? (
                  ...
                  <Radio size={16} color={
                    apiStatus === 'working' ? Colors.primary :
                    apiStatus === 'failed' ? '#EF4444' : Colors.textSecondary
                  } />
                )}
              </TouchableOpacity>
            )}
            ...
            <View>
              <View style={styles.resultsHeader}>
                <Text style={styles.resultsCount} testID="explore-results-count">
                  {searchQuery 
                    ? `${String(searchResults.length)} results found`
                    : `All ${String(speeches.length)} speeches`
                  }
                </Text>
                {searchQuery && apiStatus === 'working' && (
                  <Text style={styles.searchHint}>
                    Press antenna to search podcasts online
                  </Text>
                )}
              </View>
              {displaySpeeches
                ...
                .map((speech, index) => {
                  // Ensure we have a valid speech object
                  if (!speech || typeof speech !== 'object' || !speech.id || !speech.title) {
                    return null;
                  }
                  return (
                    <SpeechCard
                      key={`explore-speech-${speech.id}-${index}`}
                      speech={speech}
